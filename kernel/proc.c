...
found:
  ...
  //An empty user kernel page tbl
  p->kernelpgtbl = proc_kernelpgtbl(p);
  if (p->kernelpgtbl == 0)
  {
    freeproc(p);
    release(&p->lock);
  ...
  if (pa == 0)
    panic("kalloc");
  uint64 va = KSTACK((int)(p - proc));
  ukvmmap(p->kernelpgtbl, va, (uint64)pa, PGSIZE, PTE_R | PTE_W);
  p->kstack = va;
...
//create a kernel page for a given process
pagetable_t
...
//free a process's kernel process table and do not free the physical memory
